...
- `APP_PORT` – internal application port used for both the UI and API (default `443`).
- `INSTALL_NGINX` – set to `0` to skip nginx installation.

After installation the `manage-playrservers` systemd unit supervises a single
Uvicorn instance that serves both the management UI and automation API on the
same port (default `443`) over HTTPS. The installer provisions a self-signed
certificate at `config/tls/server.crt` + `server.key`; replace these files with
certificates issued by your internal CA if desired. You can reach the UI
directly from your LAN using the server's IP address (for example,
`https://192.168.1.212:443`). Expose `https://manage.playrservers.com` by
routing requests to that port. When publishing a dedicated API hostname
(`https://api.playrservers.com`), proxy the host to the same backend and forward
requests to `/api` so remote agents can continue using familiar paths such as
`/health` and `/v1/servers/connect`.
...
| `MANAGEMENT_PORT`         | Listen port for the combined UI/API service                   | `443`                         |
...
| `MANAGEMENT_TRUSTED_PROXIES` | Comma-separated list of proxy hosts whose forwarded headers are trusted | `*` |
| `MANAGEMENT_PUBLIC_API_URL` | External URL advertised to agents via the portal (include `/api` if sharing the management hostname) | `https://api.playrservers.com` |
| `MANAGEMENT_SESSION_SECRET` | Secret used to sign management sessions (must be set)        | generated by installer         |
| `MANAGEMENT_DISABLE_TLS`    | Serve the application over plain HTTP (development/testing only) | `false` |
| `MANAGEMENT_SSL_CERTFILE`   | Path to the TLS certificate used by uvicorn                     | `config/tls/server.crt` |
| `MANAGEMENT_SSL_KEYFILE`    | Path to the TLS private key                                      | `config/tls/server.key` |
| `MANAGEMENT_SSL_KEYFILE_PASSWORD` | Passphrase for the TLS private key (if encrypted)            | unset |
| `MANAGEMENT_SSL_CA_CERTS`   | Optional CA bundle to present to uvicorn for client auth        | unset |
...
The automation endpoints share the same application process as the management UI
and are exposed under the `/api` prefix. When fronting the service with nginx or
Cloudflare, route `https://api.playrservers.com` to the same backend and rewrite
requests to `/api/...` so remote agents can continue using simple paths (e.g.
`/health`, `/agents`). The paths below reflect the public API hostname; prepend
`/api` when calling the service directly. Every endpoint except `/health`
requires an API key presented in the `Authorization: Bearer <key>` header.
...
- TLS is enabled by default; replace the bundled self-signed certificate or
  configure downstream proxies to trust it before exposing the service.
...
MANAGEMENT_PORT=8000 MANAGEMENT_DISABLE_TLS=true MANAGEMENT_RELOAD=true python main.py
```

Use `scripts/create_user.py` (or import `Database` in a REPL) to create a user
locally. With TLS disabled for local development the management UI is reachable
at `http://127.0.0.1:8000` (or via your machine's LAN IP) and the API at
`http://127.0.0.1:8000/api`. When testing over plain HTTP you can set
`MANAGEMENT_SESSION_SECURE=false` to allow the browser to retain the login
session. To exercise HTTPS locally, drop a certificate and key into
`config/tls/` (or override `MANAGEMENT_SSL_CERTFILE`/`MANAGEMENT_SSL_KEYFILE`)
and omit `MANAGEMENT_DISABLE_TLS` from the launch command—browsers will warn
when connecting to the self-signed default.
