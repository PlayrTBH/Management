...
| `MANAGEMENT_AGENT_PRIVATE_KEY_PATH` | Private key copied into new hypervisor records         | `/etc/playrservers/ssh/id_ed25519` |
| `MANAGEMENT_AGENT_PUBLIC_KEY_PATH`  | Public key returned to agents (defaults to `<private>.pub`) | derived from private key path |
...
and returns the associated public key(s) from
`MANAGEMENT_AGENT_PUBLIC_KEY_PATH`. Additional environment variables control the
username, SSH port, host verification policy, and whether the agent should close
existing SSH sessions after provisioning.
