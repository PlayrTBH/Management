...
same port (default `443`). You can reach the UI directly from your LAN using the
server's IP address (for example, `http://192.168.1.212:443`). Expose
`https://manage.playrservers.com` by routing requests to that port. When
publishing a dedicated API hostname (`https://api.playrservers.com`), proxy the
host to the same backend and forward requests to `/api` so remote agents can
continue using familiar paths such as `/health` and `/v1/servers/connect`.
...
MANAGEMENT_PORT=8000 MANAGEMENT_RELOAD=true python main.py
```
...
machine's LAN IP) and the API at `http://127.0.0.1:8000/api`; target the API base
